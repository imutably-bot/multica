...
export function applyIssueFilterUrlState(
  ...
  if (normalized.dateFilter) {
    ...
    if (normalized.dateFilter.preset === "today" || normalized.dateFilter.preset === "last_days") {
      next.set(URL_KEYS.datePreset, normalized.dateFilter.preset);
      if (normalized.dateFilter.preset === "last_days") {
