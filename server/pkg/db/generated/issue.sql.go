...
const touchIssueUpdatedAt = `-- name: TouchIssueUpdatedAt :one
UPDATE issue SET
    updated_at = now()
WHERE id = $1 AND workspace_id = $2
...
type TouchIssueUpdatedAtParams struct {
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) TouchIssueUpdatedAt(ctx context.Context, arg TouchIssueUpdatedAtParams) (pgtype.Timestamptz, error) {
	row := q.db.QueryRow(ctx, touchIssueUpdatedAt, arg.ID, arg.WorkspaceID)
