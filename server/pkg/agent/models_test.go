...
// TestParseAntigravityModels covers both the current `slug<TAB>label` format
// and the legacy display-only format. IDs are the values passed to --model.
func TestParseAntigravityModels(t *testing.T) {
	...
	out := strings.Join([]string{
		...
		"",                      // blank line — skipped
		"GPT-OSS 120B (Medium)", // legacy format
		"claude-opus-4-6-thinking\tClaude Opus 4.6 (Thinking)", // duplicate ID
	}, "\n")
