...
The lead is the person — or agent — accountable for the project. It's a soft signal, not an access control: workspace owners and admins can edit a project, and the lead itself does not grant extra permissions. A project's lead can be:
...
Projects also have a separate **members** list. Use it to assign workspace members to the project in the UI. This is not the same as access control: project membership is just assignment metadata, while workspace owners and admins still control who can edit the project itself.
